...
export async function pickTheme(
  ...
  const {
    ...
    pageSize = 10,
    message =
      "Select a theme (↑/↓ navigate, type to filter, Enter to select, Esc to cancel):",
  } = options;
  ...
  function filterThemes(): void {
    ...
    scrollOffset = Math.max(
      0,
      Math.min(scrollOffset, Math.max(0, filteredThemes.length - pageSize)),
    );
  ...
  function render(): void {
    ...
    const searchDisplay = searchQuery ||
      styled("Type to filter...", fg.brightBlack, style.italic);
  ...
  function handleInput(input: string): "continue" | "select" | "cancel" {
    ...
    if (
      isKey(input, KEY.CTRL_C) ||
      (isEscapeOnly(input) && !input.startsWith("\x1b["))
    ) {
      return "cancel";
    }
    ...
    if (
      input.length === 1 && input.charCodeAt(0) >= 32 &&
